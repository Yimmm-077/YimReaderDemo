...
@interface QLReadPageViewController ()<UIPageViewControllerDelegate,UIPageViewControllerDataSource,QLMenuChapterVCDelegate>
{
...
@property (nonatomic,strong) QLTapMenuViewController * qlTapMenuVC;   //当前阅读视图
...
//    [self.catalogView addSubview:self.menuCatalogVC.view];
    ...
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(bookMark) name:QLClickMarkBtnNoti object:nil];
...
- (QLTapMenuViewController *)qlTapMenuVC
{
    if (!_qlTapMenuVC)
    {
        // 2版
        _qlTapMenuVC = [[QLTapMenuViewController alloc]init];
        _qlTapMenuVC.catalogDelegate = self;
        //    qlTapMenuVC.delegate = self;
        _qlTapMenuVC.RBmodel = _RBmodel;
        _qlTapMenuVC.modalPresentationStyle = UIModalPresentationOverFullScreen;
    }
    return _qlTapMenuVC;
}
...
-(void)bookMark
{
    ...
    NSString * key = [NSString stringWithFormat:@"%d_%d",(int)_RBmodel.RBrecord.chapter,(int)_RBmodel.RBrecord.page];
    id state = _RBmodel.marksRecord[key];
    if (state) {
//如果存在移除书签信息
        [_RBmodel.marksRecord removeObjectForKey:key];
        [[_RBmodel mutableArrayValueForKey:@"marks"] removeObject:state];
    }
    else{
//记录书签信息
        QLMarkModel * model = [[QLMarkModel alloc] init];
        model.date = [NSDate date];
        model.recordModel = [_RBmodel.RBrecord copy];
        [[_RBmodel mutableArrayValueForKey:@"marks"] addObject:model];
        [_RBmodel.marksRecord setObject:model forKey:key];
    }
    
    _qlTapMenuVC.tapMenuView.topMenu.state = !state;
...
//    [_readView.readView cancelSelected];
    ...
    NSString * key = [NSString stringWithFormat:@"%d_%d",(int)_RBmodel.RBrecord.chapter,(int)_RBmodel.RBrecord.page];
    id state = _RBmodel.marksRecord[key];
    state?(_qlTapMenuVC.tapMenuView.topMenu.state=1): (_qlTapMenuVC.tapMenuView.topMenu.state=0);
    
//    [self.tapMenuView showAnimation:YES];
    

    [self presentViewController:self.qlTapMenuVC animated:NO completion:nil];
